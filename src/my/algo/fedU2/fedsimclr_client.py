...
class SimclrEvenClient:
    ...
    def fit(self, global_model) -> dict[str, float]:
        self.model.load_state_dict(global_model.state_dict()) # careful byol update by modules
        self.optimizer = self.configure_optimizer()
        ...
        for epoch in range(E):
            if not self.use_deg:
                for x1, x2 in self.aug_train_loader:
                    ...
                    h1, h2, z1, z2 = self.model(x1, x2)

                    # decorr_loss = self.fedcorr(h1) + self.fedcorr(h2)
                    uuotr = self.recollapse_tagent_loss(h1) + self.recollapse_tagent_loss(h2)
                    # ccassg = self.ccassg_loss(h1, h2)
                    loss = nt_xent_loss(h1, h2, self.temperature) + 0.01 * uuotr
                ...
                for x1, x2, x3, deg_labels in self.aug_train_loader:
                    ...
                    h1, h2, z1, z2, deg_preds = self.model(x1, x2, x3)
                    L_deg = torch.nn.functional.cross_entropy(deg_preds, deg_labels)
                    uuotr = self.recollapse_tagent_loss(h1) + self.recollapse_tagent_loss(h2)
                    loss = nt_xent_loss(h1, h2, self.temperature) + 0.01 * uuotr + L_deg
                    self.optimizer.zero_grad()
                    loss.backward()
                    self.optimizer.step()
                    losses.append(loss.item())
