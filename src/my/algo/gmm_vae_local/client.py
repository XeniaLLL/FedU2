...
class GMMVaeLocalClient(EvalMetric):
    def __init__(
            self,
            *,
            id: int,
            ...
            mean_mlp: nn.Module,
            var_mlp: nn.Module,
            prior_mean_mlp: nn.Module,
            prior_var_mlp: nn.Module,
            y_block: nn.Module,
            y_logits: nn.Module,
            h_top_mlp: nn.Module,
            h_latent_mlp: nn.Module,
            decoder: nn.Module,
            cluster_num: int,
            hidden_size: int,
        ...
        self.var_mlp = var_mlp
        self.prior_mean_mlp = prior_mean_mlp
        self.prior_var_mlp = prior_var_mlp
        ...
        self.h_top_mlp = h_top_mlp

        self.h_latent_mlp = h_latent_mlp
    ...
    def configure_optimizer(self):
        return torch.optim.Adam([
            ...
            *self.mean_mlp.parameters(),
            *self.var_mlp.parameters(),
    ...
    def loss_compute(self, data):
        ...
        l_y_kl = torch.mean(py * (torch.log(py + 1e-8) - torch.log(torch.tensor(1. / self.cluster_num))))  # model.y_size== cluster_num careful<- assumption
    ...
    def fit(self) -> dict[str, float]:
        ...
        for epoch in tqdm(range(self.local_epochs), desc=f'client {self.id}', leave=False):
            for data in tqdm(self.train_loader, desc=f'epoch {epoch}', leave=False):
                ...
                l_x_recon, l_x_kl, l_y_kl = self.loss_compute(data)
                total_loss = l_x_recon + self.beta * l_x_kl + self.alpha * l_y_kl
                self.optimizer.zero_grad()
                total_loss.backward()
                self.optimizer.step()
                train_losses.append(total_loss.item())
        return {'train_loss': np.mean(train_losses).item()}
