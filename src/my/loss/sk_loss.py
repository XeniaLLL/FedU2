...
from geomloss.utils import distances, squared_distances, scal
...
# ==============================================================================
#                             call div loss functions: tensorized sample backend
# ==============================================================================
...
def sinkhorn_tensorized(
    ...
    if a is None:
        a= torch.ones((B, N)).to(x) /N

    if b is None:
        b= torch.ones((B,M)).to(x) /M

    # note added by Xenia
    pi_xy_pre = torch.ones((B, N, M)).to(x) / (M * N)  # B batch size

    pi_xx_pre = torch.ones((B, N, N)).to(x)  / (N * N)  # B batch size

    pi_yy_pre = torch.ones((B, M, M)).to(x)  / (M * M)  # B batch size
...
def sinkhorn_loop(
    ...
    # note modified by xenia step1. 求对偶变量的时候,初始化cost为(C+ log\pi)
    C_xy, C_yx = C_xys[k] + eps * pi_xy_pre.log(), C_yxs[k] + eps * pi_yx_pre.log()  # C(x_i, y_j), C(y_i, x_j)
    if debias:  # Info for the "a <-> a" and "b <-> b" problems
        C_xx, C_yy = C_xxs[k] + eps * pi_xx_pre.log(), C_yys[k] + eps * pi_yy_pre.log()  # C(x_i, x_j), C(y_j, y_j)
    ...
    # note added by xenia: step2. 用新的对偶变量更新\pi

    K_xy= torch.exp(-C_xy/ eps)
    K_xx= torch.exp(-C_xx/ eps)
    K_yy= torch.exp(-C_yy/ eps)

    pi_xy_pre = torch.matmul(torch.matmul(torch.diag(f_ba.squeeze()).unsqueeze(0), K_xy), torch.diag(g_ab.squeeze()).unsqueeze(0))
    pi_yx_pre = pi_xy_pre.transpose(2,1)
    pi_xx_pre = torch.matmul(torch.matmul(torch.diag(f_aa.squeeze()).unsqueeze(0), K_xx), torch.diag(f_aa.squeeze()).unsqueeze(0))
    pi_yy_pre = torch.matmul(torch.matmul(torch.diag(g_bb.squeeze()).unsqueeze(0), K_yy), torch.diag(g_bb.squeeze()).unsqueeze(0))
    # note step3. 更新pi对应新的cost
    C_xy, C_yx = C_xys[k] + eps * pi_xy_pre.log(), C_yxs[k] + eps * pi_yx_pre.log()  # C(x_i, y_j), C(y_i, x_j)
    if debias:  # Info for the "a <-> a" and "b <-> b" problems
        C_xx, C_yy = C_xxs[k] + eps * pi_xx_pre.log(), C_yys[k] + eps * pi_yy_pre.log()  # C(x_i, x_j), C(y_j, y_j)
    ...
    for i, eps in enumerate(eps_list):  # See Fig. 3.25-26 in Jean Feydy's PhD thesis.
        ...
        #       Sinkhorn formulas, and update both dual vectors # note 更新对偶变量不更新新的pi
        #       simultaneously.
        ...
        # note added by xenia: step4 迭代优化对偶变量后,对应更新pi
        K_xy = torch.exp(-C_xy / eps)
        if debias:
            K_xx = torch.exp(-C_xx / eps)
            K_yy = torch.exp(-C_yy / eps)
    ...
    # note step last 计算最后对偶变量对应的mapping
    # K_xy = torch.exp(-C_xy / eps)
    # if debias:
    #     K_xx = torch.exp(-C_xx / eps)
    #     K_yy = torch.exp(-C_yy / eps)

    # pi_xy = torch.matmul(torch.matmul(torch.diag(f_ba), K_xy), torch.diag(g_ab))
    # pi_yx = pi_xy_pre.T
    if debias:
        # pi_xx = torch.matmul(torch.matmul(torch.diag(f_aa), K_xx), torch.diag(f_aa))
        # pi_yy = torch.matmul(torch.matmul(torch.diag(g_bb), K_yy), torch.diag(g_bb))
        return f_aa, g_bb, g_ab, f_ba
    else:
        return None, None, g_ab, f_ba, pi_xy_pre, pi_yx_pre
